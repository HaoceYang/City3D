...
vec2 PaintCanvas::projectionOf(const vec3& p)
{
    // point to screen
...
vec3 PaintCanvas::unProjectionOf(double winx, double winy, double winz)
{
    // screen to point
...
void PaintCanvas::reconstruct()
{
    ...
    bool status = recon.reconstruct(point_set_, footprint_, reconstruction_, main_window_->active_solver(),
                                    show_reconstruction_);
    if (!status)
